...
import { readGroupTransactionsGroupsGroupIdTransactionsGet, type Group, type TransactionRead } from '$lib/client';
import type { Balance } from '$lib/interfaces/balance';
import type { SuperValidated } from 'sveltekit-superforms';
import type { zUpdateGroup } from '$lib/client/zod.gen';
import type { z } from 'zod';

async function getPageData(fetch: Fetch, groupId: string|null, cookies: Cookies): Promise<{
	balances: Balance[],
...
export const load: PageServerLoad = async ({ fetch, url, cookies }): Promise<{
	balances: Balance[],
	transactions: TransactionRead[]
}|{}> => {
...
export const actions: Actions|undefined = isCompiledStatic() ? undefined : {
	data_layout: async ({ request }): Promise<{
		groupData: Group,
	...
	data: async ({ fetch, request, cookies }): Promise<{
		balances: Balance[],
		transactions: TransactionRead[]
	}> => {
		const data = await request.formData();
