<script lang="ts">
	...
	import { login } from '$lib/shared/auth/flows';
	...
	const { form, errors, enhance, message, constraints, submitting } = superForm(data.loginForm, {
		onUpdate: async ({ form, cancel }) => {
			...
			try {
				await login(form.data.email as string, form.data.password as string);
				await goto('/');
			} catch (e) {
				message.set(e instanceof Error ? e.message : 'Invalid email or password');
				cancel();
			}
		}
	});
